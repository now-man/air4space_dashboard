...
import { Zap, Settings, ShieldAlert, BotMessageSquare, Plus, Trash2, Save, ArrowLeft, UploadCloud, TestTube2, BrainCircuit, Eraser, Lightbulb, RefreshCw, PlayCircle, MapPin, Edit3, Compass, Activity, Calendar as CalendarIcon, MoreVertical, X, Edit } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    fetch(FETCH_URL)
        ...
        .then(csvText => {
            ...
            if (allData.length === 0) throw new Error('CSV data is empty or invalid after parsing.');
            const todayKey = formatDateKey(new Date());
            const todaysData = allData.filter(d => d && d.time && d.time.startsWith(todayKey) && d.time.includes('T'));
            if (todaysData.length === 0) {
                 setForecastStatus({ isLoading: false, error: "오늘 날짜에 해당하는 예측 데이터가 없습니다." });
                 setForecastData([]);
                 return;
            }
            const formattedData = todaysData.map(d => ({
                time: d.time.split('T')[1].substring(0, 5),
                predicted_error: parseFloat(d.gnss_error) || 0,
                kp_index: parseFloat(d.tec) || 0
            }));
  ...
  const renderView = () => {
    switch (activeView) {
      ...
      default: return <DashboardView profile={unitProfile} forecast={forecastData} forecastStatus={forecastStatus} logs={missionLogs} deleteLog={deleteLog} todoList={todoList} addTodo={addTodo} updateTodo={updateTodo} deleteTodo={deleteTodo} />;
    }
  };
  
  return (
    <div className="bg-gray-900 text-gray-200 min-h-screen font-sans">
      <Header profile={unitProfile} setActiveView={setActiveView} activeView={activeView} />
...
const TodoList = ({ todoList, addTodo, updateTodo, deleteTodo }) => {
    ...
    const handleAdd = () => {
        ...
        if(text) { addTodo({time, text, tag: 'Briefing'}); document.getElementById('todoText').value = ''; }
    };
    const handleSave = (id) => {
        const time = document.getElementById(`edit-time-${id}`).value;
        const text = document.getElementById(`edit-text-${id}`).value;
        updateTodo({ ...editingTodo, time, text });
        setEditingTodo(null);
    };
    ...
    <>
        {menuTodo && (
            <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50" onClick={() => setMenuTodo(null)}>
                <div className="bg-gray-800 border border-gray-600 rounded-lg p-6 w-full max-w-xs space-y-4" onClick={e => e.stopPropagation()}>
                    <p className="text-lg font-semibold text-white text-center">"{menuTodo.text}"</p>
                    <div className="flex flex-col space-y-3">
                        <button onClick={handleEditClick} className="w-full text-left px-4 py-2.5 text-sm bg-gray-700 hover:bg-gray-600 rounded-md flex items-center gap-3 transition-colors"><Edit size={16}/> 수정하기</button>
                        <button onClick={handleDeleteClick} className="w-full text-left px-4 py-2.5 text-sm text-red-400 bg-red-900/50 hover:bg-red-900/80 rounded-md flex items-center gap-3 transition-colors"><Trash2 size={16}/> 삭제하기</button>
                    </div>
                </div>
            </div>
        )}
...
// --- Dashboard View ---
const DashboardView = ({ profile, forecast, forecastStatus, logs, deleteLog, todoList, addTodo, updateTodo, deleteTodo }) => {
    ...
    const DayContentWithDots = (props) => {
        ...
        return (<div className="relative w-full h-full flex justify-center items-center"><span>{props.date.getDate()}</span>{dots.length > 0 && (<div className="absolute bottom-1.5 flex space-x-0.5">{dots.map((dotClass, i) => (<div key={i} className={`w-1.5 h-1.5 rounded-full ${dotClass}`}></div>))}</div>)}</div>);
    };
                ...
                <div className="bg-gray-800 p-4 md:p-6 rounded-xl border border-gray-700">
                    ...
                    <div style={{width: '100%', height: 250}}>
                        {forecastStatus.isLoading ? (<div className="flex items-center justify-center h-full text-gray-400">데이터 로딩 중...</div>)
                         ...
                         : (<ResponsiveContainer width="100%" height="100%"><LineChart data={forecast}><CartesianGrid strokeDasharray="3 3" stroke="#4A5568" /><XAxis dataKey="time" stroke="#A0AEC0" /><YAxis yAxisId="left" label={{ value: 'GNSS 오차(m)', angle: -90, position: 'insideLeft', fill: '#A0AEC0' }} stroke="#F56565" /><YAxis yAxisId="right" orientation="right" label={{ value: 'TEC (TECU)', angle: 90, position: 'insideRight', fill: '#A0AEC0' }} stroke="#4299E1" /><Tooltip contentStyle={{ backgroundColor: '#1A202C' }} /><Legend /><Line yAxisId="left" dataKey="predicted_error" name="GNSS 오차" stroke="#F56565" dot={false} /><Line yAxisId="right" dataKey="kp_index" name="TEC" stroke="#4299E1" dot={false} /><ReferenceLine yAxisId="left" y={activeUnitThreshold} label={{ value: "부대 임계값", fill: "#4FD1C5" }} stroke="#4FD1C5" strokeDasharray="4 4" /></LineChart></ResponsiveContainer>)}
                    </div>
                </div>
