...
const Header = ({ profile, setActiveView, activeView, onWeatherClick, apiKey }) => {
...
const LiveMap = ({threshold, center, isRaining, apiKey}) => {
...
const FeedbackMap = ({ data, equipment, isAnimating, animationProgress, showClouds, isRaining, apiKey }) => {
    ...
    const cloudOpacity = isAnimating ? 0.3 + 0.5 * Math.abs(Math.sin(animationProgress * Math.PI * 10)) : 0.8;
    ...
    return (
        <div className="h-56 rounded-lg overflow-hidden relative">
            <MapContainer center={data[0] ? [data[0].lat, data[0].lon] : [36.6, 127.4]} zoom={11} style={{ height: "100%", width: "100%" }} scrollWheelZoom={false}>
                ...
                {(showClouds || isRaining) && apiKey && (
                    <>
                        <TileLayer key={`clouds-${isAnimating}`} url={`https://tile.openweathermap.org/map/clouds_new/{z}/{x}/{y}.png?appid=${apiKey}`} attribution='&copy; OpenWeatherMap' zIndex={2} opacity={isAnimating ? cloudOpacity : 0.5}/>
                        <TileLayer className="weather-tile-layer" key={`precip-${Math.floor(animationProgress * 10)}`} url={`https://tile.openweathermap.org/map/precipitation_new/{z}/{x}/{y}.png?appid=${apiKey}`} attribution='&copy; OpenWeatherMap' zIndex={3} opacity={isRaining ? 0.9 : (isAnimating ? cloudOpacity : 0.7)}/>
                    </>
                )}
...
const XAIAnalysisReport = ({ allForecastData, threshold }) => {
    const analysis = useMemo(() => {
        ...
        const maxErrorPoint = relevantData.reduce((max, p) => p.fore_gnss > max.fore_gnss ? p : max, { fore_gnss: -1 });
        const maxError = maxErrorPoint.fore_gnss;

        const factors = [];
        if (maxErrorPoint.kp >= 5) factors.push({ name: "Kp 지수", value: maxErrorPoint.kp.toFixed(1), cause: "지자기 폭풍", icon: <Wind size={16} className="text-yellow-400"/> });
        if (maxErrorPoint.xrsb > 1e-5) factors.push({ name: "X선 플럭스", value: maxErrorPoint.xrsb.toExponential(1), cause: "태양 플레어", icon: <Sun size={16} className="text-red-400"/> });
        if (maxErrorPoint.tec_value > 50) factors.push({ name: "총 전자 함유량(TEC)", value: maxErrorPoint.tec_value.toFixed(1), cause: "전리층 불안정", icon: <Zap size={16} className="text-blue-400"/>});

        let status, recommendation;
        
        if (maxError > threshold) {
            status = { text: "위험", className: "text-red-400" };
            recommendation = "정밀 타격 등 GNSS 의존도가 높은 임무 수행 시 각별한 주의가 필요하며, 대체 항법 수단 사용을 적극 고려해야 합니다.";
        } else if (maxError > threshold * 0.7) {
            status = { text: "주의", className: "text-yellow-400" };
            recommendation = "GNSS 민감 장비 운용 시 간헐적 오차 증가에 대비하고, 대체 항법 수단을 숙지하십시오.";
        } else {
            status = { text: "안정", className: "text-green-400" };
            recommendation = "모든 임무를 정상적으로 수행할 수 있습니다.";
        }
        
        const conclusion = `향후 24시간 내 최대 GNSS 오차는 **${formatDate(maxErrorPoint.timestamp, 'time')}경 ${maxError.toFixed(2)}m**로 예측됩니다. 이는 부대 임계값(${threshold.toFixed(2)}m) 대비 <strong class="${status.className}">${status.text}</strong> 수준입니다.`;
    ...
    return (
        <div className="bg-gray-800 p-4 md:p-6 rounded-xl border border-gray-700">
            ...
            <div className="space-y-4 text-sm">
                <div>
                    <p className="font-semibold text-gray-300 mb-1">종합 분석</p>
                    <p className="text-gray-400 leading-relaxed" dangerouslySetInnerHTML={{ __html: analysis.conclusion.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>') }}></p>
                </div>
                {analysis.factors.length > 0 && (
                    <div>
                        <p className="font-semibold text-gray-300 mb-1">주요 영향 요인 (최대 오차 시점 기준)</p>
                ...
                <div>
                    <p className="font-semibold text-gray-300 mb-1">권고 사항</p>
...
const OptimalTimeRecommender = ({ allForecastData, onRecommendation, activeUnitThreshold }) => {
    ...
    const handleRecommend = () => {
        ...
        for (let i = 0; i < relevantData.length; i++) {
            const windowStart = relevantData[i].timestamp;
            const windowEnd = windowStart + durationInMillis;

            // *** 중요: 작전 종료 시간이 탐색 범위를 벗어나면 중단 ***
            if (windowEnd > searchEndTs) {
                break;
            }

            const currentWindow = relevantData.filter(d => d.timestamp >= windowStart && d.timestamp < windowEnd);

            if (currentWindow.length > 0) {
                const currentSum = currentWindow.reduce((sum, d) => sum + d.fore_gnss, 0);
                const currentAvg = currentSum / currentWindow.length;
                
                if (currentAvg < minAvgError) {
                    minAvgError = currentAvg;
        ...
        if (bestWindow && bestWindow.length > 0) {
            ...
            const recommendedEnd = bestStartTime + durationInMillis;
            const avgError = minAvgError;
            const maxError = Math.max(...bestWindow.map(d => d.fore_gnss));
            const avgKp = bestWindow.reduce((s, d) => s + d.kp, 0) / bestWindow.length;
            
            setResult({
                time: `추천 작전 시간: ${formatDate(bestStartTime, 'full')} ~ ${formatDate(recommendedEnd, 'full')}`,
                analysis: { avgError, maxError, avgKp, kpStatus: getKpStatus(avgKp) },
                error: null
            });
            onRecommendation({start: bestStartTime, end: recommendedEnd});
        } else {
            setResult({ error: "선택한 기간 내에 추천 가능한 작전 시간대를 찾을 수 없습니다." });
            onRecommendation(null);
        }
    };
            ...
            {result && !result.error && (
                <div className="mt-4 pt-4 border-t border-gray-700 space-y-3">
                    ...
                    <div className="bg-gray-900/50 p-3 rounded-md text-sm">
                         <p className="text-gray-300 mb-2" dangerouslySetInnerHTML={{ __html: `해당 시간대는 **Kp 지수가 <span class="${result.analysis.kpStatus.color}">${result.analysis.kpStatus.text}</span> 상태**를 유지하고, **평균 예측 오차가 ${formatNumber(result.analysis.avgError)}m**로 가장 낮아 작전 수행에 최적의 환경을 제공합니다.` }}></p>
...
const FutureMissionPlanner = ({ allForecastData, profile }) => {
    ...
    const handlePredict = () => {
        ...
        const avgKp = relevantData.reduce((s, d) => s + d.kp, 0) / relevantData.length;


        // 성공률 계산 로직 (가중치 기반)
        const maxErrorScore = Math.max(0, (1 - (maxError / (threshold * 2)))) * 40; // 최대 오차가 임계값 2배일때 0점
        const riskRatioScore = (100 - riskRatio) * 0.4; // 위험비율 100%일때 0점
        const avgKpScore = Math.max(0, (1 - (avgKp / 9))) * 20; // Kp 9일때 0점
        
        let successRate = maxErrorScore + riskRatioScore + avgKpScore;
        ...
        setPrediction({
            ...
            basis: {
                maxErrorRatio: formatNumber((maxError / threshold) * 100, 0),
                avgKp: formatNumber(avgKp, 1)
            },
            error: null,
        });
    };
                ...
                <div className="flex flex-col gap-4">
                    {prediction && !prediction.error && (
                        <div className='space-y-3'>
                            ...
                            <div className="text-xs text-gray-400 bg-gray-900/50 p-3 rounded-lg space-y-1">
                                <p className='font-bold text-gray-300 text-sm mb-2'>분석 근거</p>
                                <li className="flex justify-between"><span>최대 오차/임계값:</span> <strong>{prediction.basis.maxErrorRatio}%</strong></li>
                                <li className="flex justify-between"><span>임무 시간 내 위험 비율:</span> <strong>{prediction.riskRatio}%</strong></li>
                                <li className="flex justify-between"><span>평균 Kp 지수:</span> <strong>{prediction.basis.avgKp} ({getKpStatus(prediction.basis.avgKp).text})</strong></li>
                            </div>
                        </div>
                    )}
...
const SettingsView = ({ profiles, setProfiles, activeProfile, setActiveProfileId, goBack, createDefaultProfile, onRecalculate }) => {
    ...
    const handleRecalculateAndUpdateLocal = () => {
        const updatedProfiles = onRecalculate(); // onRecalculate가 업데이트된 profiles를 반환하도록 수정
        if (updatedProfiles) {
            const updatedActiveProfile = updatedProfiles.find(p => p.id === activeProfile.Id);
            setLocalProfile(JSON.parse(JSON.stringify(updatedActiveProfile)));
        }
    };
        ...
        <div className="space-y-6">
            ...
            <div>
                ...
                <div className="flex space-x-4">
                    ...
                    <button onClick={() => downloadCSV(csvContentGeo, 'strike_mission_geo_0923_v2.csv')} className="w-full bg-cyan-600 hover:bg-cyan-700 text-white font-semibold py-2 px-4 rounded-lg flex items-center justify-center space-x-2"><UploadCloud size={20} /><span>위경도 포함 로그 (.csv)</span></button>
                </div>
            </div>
            <div>
    ...
    return (<>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-6">
                ...
                <OptimalTimeRecommender allForecastData={allForecastData} onRecommendation={setRecommendedRange} activeUnitThreshold={activeUnitThreshold}/>
...
                if (errRatesOnFailure.length >= 10) {
...
    const unitAutoThreshold = useMemo(() => activeProfile.unitAutoThreshold || 20.0, [activeProfile.unitAutoThreshold]);
...
}%%writefile /content/drive/MyDrive/air4space/result/system/air4space_dashboard/src/App.js
import React, { useState, useEffect, useMemo, useRef } from 'react';
import { createPortal } from 'react-dom';
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, ReferenceLine, ReferenceArea, BarChart, Bar, ScatterChart, Scatter, Cell, Label } from 'recharts';
import { DayPicker } from 'react-day-picker';
