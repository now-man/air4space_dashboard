...
        let conclusion = `24시간 내 최대 GNSS 오차는 <strong>${maxError.toFixed(2)}m</strong>로 예측됩니다. 이는 부대 임계값 ${threshold.toFixed(2)}m 대비 `;
        let recommendation;

        if (maxError > threshold) {
            conclusion += "<strong>위험</strong> 수준입니다.";
            recommendation = "정밀 타격 및 GNSS 의존도가 높은 임무 수행 시 각별한 주의가 필요하며, 대체 항법 수단 사용을 적극 고려해야 합니다.";
        } else if (maxError > threshold * 0.7) {
            conclusion += "<strong>주의</strong> 수준입니다.";
            recommendation = "GNSS 민감 장비 운용 시 간헐적 오차 증가에 대비하고, 대체 항법 수단을 숙지하십시오.";
        } else {
            conclusion += "<strong>안정</strong> 수준입니다.";
...
                    <p className="text-gray-400" dangerouslySetInnerHTML={{ __html: analysis.conclusion.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>') }}></p>
                </div>
