...
const WeatherForecastPopover = ({ profile, apiKey, onClose, position }) => {
    ...
    return createPortal(
        <div ref={popoverRef} className="absolute z-50 bg-gray-800 border border-gray-600 rounded-xl p-4 w-full max-w-md shadow-lg" style={{ top: `${position.bottom + 8}px`, right: `${window.innerWidth - position.right}px`}}>
            <h2 className="text-lg font-bold text-white mb-3">시간대별 예보</h2>
            {loading ? <p className="text-gray-300">로딩 중...</p> : !forecast ? <p className="text-red-400">예보를 불러올 수 없습니다.</p> :
            <div className="space-y-2 max-h-80 overflow-y-auto">
                {forecast.map(hour => (
                    <div key={hour.dt} className="grid grid-cols-4 items-center text-center p-2 bg-gray-900/50 rounded-md text-gray-200">
...
const LiveMap = ({threshold, center, isRaining}) => {
    ...
    return (<div className="bg-gray-800 p-4 md:p-6 rounded-xl border border-gray-700 h-96 flex flex-col"><h2 className="text-lg font-semibold mb-4 text-white">실시간 항적 및 기상</h2><div className="flex-grow relative"><MapContainer key={center.lat + "-" + center.lon} center={[center.lat, center.lon]} zoom={9} style={{ height: "100%", width: "100%", borderRadius: "0.75rem", backgroundColor: "#333" }}> <TileLayer url="https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png" attribution='&copy; CARTO' /> {(showClouds || isRaining) && OWM_API_KEY !== "5e51e99c2fa4d10dbca840c7c1e1781e" && (
        <>
            {isRaining && <TileLayer url={`https://tile.openweathermap.org/map/clouds_new/{z}/{x}/{y}.png?appid=${OWM_API_KEY}`} attribution='&copy; OpenWeatherMap' zIndex={2} opacity={0.4}/>}
