...
const ShapeIcon = ({ shape, color = "white" }) => {
    ...
    switch(shape) {
        ...
        case 'hexagon': return <Hexagon {...props} fill={color} />; case 'aperture': return <Aperture {...props} fill={color} />;
                ...
                {forecast.map(hour => (
                    <div key={hour.dt} className="grid grid-cols-4 items-center text-center p-2 bg-gray-900/50 rounded-md text-white">
                        ...
                        <div className="flex items-center justify-center gap-1">
                            ...
                            <span className="w-20 text-xs">{hour.weather[0].description}</span>
                        </div>
    ...
    const analysisResult = useMemo(() => {
        ...
        const chartData = log.gnssErrorData.map(realDataPoint => {
            ...
            const closestForecast = forecastInRange.reduce((prev, curr) =>
                Math.abs(curr.timestamp - realTimestamp) < Math.abs(prev.timestamp - realTimestamp) ? curr : prev, {timestamp: Infinity}
            );
                ...
                <ResponsiveContainer width="100%" height={300}>
                    <LineChart data={analysisResult.chartData}>
                        ...
                        <XAxis
                            ...
                            tickFormatter={(unixTime) => new Date(unixTime).toLocaleTimeString('ko-KR', {hour: '2-digit', minute:'2-digit'})}
                            stroke="#A0AEC0"
                        />
...
export default function App() {
    ...
    const handleWeatherClick = (position) => {
        ...
        setIsWeatherPopoverOpen(prev => !prev);
    };
