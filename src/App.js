...
const ForecastGraph = ({ allForecastData, forecastStatus, activeUnitThreshold }) => {
    ...
    useEffect(() => {
        if (allForecastData && allForecastData.length > 0) {
            ...
            // Default to showing -12h to +24h from now, clamped by available data
            const defaultStart = now - 12 * 60 * 60 * 1000;
            ...
            setTimeRange({
                start: Math.max(defaultStart, dataMin),
                end: Math.min(defaultEnd, dataMax)
            });
        }
    }, [allForecastData]);
                ...
                {forecastStatus.isLoading ? (<div className="flex items-center justify-center h-full text-gray-400">데이터 로딩 중...</div>)
                 ...
                 : (<ResponsiveContainer width="100%" height="100%">
                     <LineChart data={displayData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                        ...
                        {/* === FIX: Added yAxisId="left" to the vertical ReferenceLine === */}
                        <ReferenceLine yAxisId="left" x={nowTimestamp} stroke="#fbbf24" strokeWidth={2} label={{ value: '현재', position: 'insideTop', fill: '#fbbf24' }} ifOverflow="extendDomain" />
                    </LineChart>
                  </ResponsiveContainer>)}
            </div>
...
const AnalysisView = ({ logs, profile }) => {
    const analysisData = useMemo(() => {
        ...
        logs.forEach(log => {
            ...
            const weekStart = new Date(date.setDate(date.getDate() - (date.getDay() || 7) + 1)).toISOString().slice(0, 10); // Monday as start of week
    ...
    if (!analysisData) {
        return <div className="text-center text-gray-400 p-8">분석할 피드백 데이터가 없습니다. [설정 > 개발자 테스트] 에서 테스트 데이터를 생성할 수 있습니다.</div>;
    }
